...
export function CartDrawer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Drawer.Root direction="right">
      ...
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 z-[100] bg-ink/48 backdrop-blur-sm" />
        <Drawer.Content className="fixed bottom-0 right-0 z-[101] flex h-full w-full max-w-md flex-col bg-ivory shadow-2xl outline-none">
          <div className="p-8 flex items-center justify-between border-b border-ink/5">
            ...
            <Drawer.Close className="flex h-10 w-10 items-center justify-center border border-ink/8 bg-white text-ink/40 transition-colors hover:text-ink">
              <X size={20} />
            </Drawer.Close>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center">
                <div className="mb-6 flex h-20 w-20 items-center justify-center border border-ink/8 bg-white text-ink/12">
                   <ShoppingBag size={32} />
                </div>
              ...
              <div className="space-y-8">
                {items.map((item) => (
                  <div key={`${item.productId}:${item.variantId || 'base'}:${item.engraving || 'plain'}`} className="flex gap-6 group">
                    <div className="h-24 w-20 shrink-0 overflow-hidden border border-ink/8 bg-ivory-2">
                      {item.imageUrl ? (
                        <Image 
                          src={item.imageUrl} 
                          alt={item.title} 
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center gap-3 border border-ink/5 bg-white px-2 py-1">
          ...
          {items.length > 0 && (
            <div className="p-8 bg-ivory-2/50 border-t border-ink/5 space-y-6">
              ...
              <Link 
                href="/checkout" 
                className="flex w-full items-center justify-center gap-3 bg-ink py-5 font-mono text-[11px] uppercase tracking-[0.2em] text-ivory shadow-xl transition-all hover:bg-ink-2 active:scale-[0.98]"
              >
