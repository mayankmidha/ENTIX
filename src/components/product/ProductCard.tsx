  ...
  return (
    <article className="group transition duration-500 motion-safe:hover:-translate-y-1">
      <div className="relative block aspect-[4/5] overflow-hidden border border-ink/8 bg-[#eee8de] shadow-[0_18px_46px_rgba(18,15,13,0)] transition-shadow duration-500 group-hover:shadow-[0_22px_70px_rgba(18,15,13,0.10)]">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-3 p-3">
          <div className="flex flex-col gap-2">
            ...
            {hasStockSignal && product.inventory.stockQty <= 4 && product.inventory.stockQty > 0 && (
              <span className="w-fit border border-oxblood/15 bg-ivory/72 px-3 py-1.5 font-subhead text-[9px] uppercase tracking-[0.16em] text-oxblood backdrop-blur-md">
                Low stock
              </span>
            )}
          </div>
        ...
        <div className="absolute bottom-3 left-3 z-10 hidden items-center gap-2 border border-white/35 bg-white/55 px-3 py-2 font-subhead text-[9px] uppercase tracking-[0.14em] text-ink/70 backdrop-blur-md transition-all group-hover:text-ink sm:flex">
          <Eye size={13} />
          View piece
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-[1fr_auto] items-center border-t border-ink/8 pt-3 font-subhead text-[9px] uppercase tracking-[0.14em] text-ink/32">
